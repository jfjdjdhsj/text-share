...
export default function PastePage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (res.ok && !data.requiresPassword && !data.expired) {
        ...
        setError((data as any)?.message ?? "加载失败");
      }
    })();
  }, [id]);
  ...
  async function unlock() {
    ...
    try {
      ...
      setFiles(data.files || []);
      setMeta((m) =>
        m ? { ...m, requiresPassword: false, expired: false, remainingViews: data.remainingViews ?? m.remainingViews } : m
      );
    } catch (e: any) {
      setError(e.message);
    } finally {
